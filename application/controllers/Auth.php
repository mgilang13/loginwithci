...
class Auth extends CI_Controller {
	...
	private function _login()
	{
		...
		$user = $this->auth_model->fit_user($email);

		if($user) {
			//jika usernya ada
			if($user['is_active'] == 1) {
				//cek password
				$cekpassword = $this->auth_model->cek_password($user, $password);
				...
				$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
											Sorry! Your email has not activated!
											</div>');
				redirect('auth');
			}
		} else {
	...
	public function registration()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->auth_model->input_to_db($data);
